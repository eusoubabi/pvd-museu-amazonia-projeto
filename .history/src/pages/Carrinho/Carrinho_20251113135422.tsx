...
export const CompraCatalogo = () => {
  ...
  // Estado para rastrear quais IDs foram clicados (selecionados)
  ...
  return (
    <div className="catalogo-container">
      ...
      <div className="ticket-grid">
        {TICKET_TYPES.map(ticket => (
          <div className="ticket-card" key={ticket.id}>
            ...
            <button 
              className={`ticket-buy-btn ${selectedItems[ticket.id] ? 'selected-btn' : ''}`} 
              onClick={() => handleAddToCart(ticket)}>
              {selectedItems[ticket.id] ? 'SELECIONADO' : 'SELECIONAR'}
            </button>
          </div>
        ))}
      </div>
      ...
      <button 
        className="hero-btn cart-redirect-btn" 
